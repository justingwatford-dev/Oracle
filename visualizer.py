...
class Visualizer:
    ...
    def generate_diagnostic_overlay(self, frame, storm_tracker, data_interface):
        ...
        # V5 FIX: Convert GPU (CuPy) arrays to CPU (NumPy) arrays
        try:
            # Check if arrays have .get() method (CuPy arrays)
            if hasattr(data_interface.u_target, 'get'):
                u_steer = data_interface.u_target.get()  # GPU -> CPU
                v_steer = data_interface.v_target.get()  # GPU -> CPU
            else:
                # Already NumPy arrays
                u_steer = data_interface.u_target
                v_steer = data_interface.v_target
        except Exception as e:
            print(f"    WARNING: Could not get steering vectors: {e}")
            u_steer = np.zeros((self.sim.nx, self.sim.ny))
            v_steer = np.zeros((self.sim.nx, self.sim.ny))
        ...
        if hasattr(storm_tracker, 'storm_path_geo_smoothed') and storm_tracker.storm_path_geo_smoothed:
            ...
            print(f"    WARNING: No simulated track data available yet")

        # Formatting
        ax.set_title(f"V5 Diagnostic Overlay | Frame {frame}")
